...
import time
from util.util import complex_matmul, complex_conj, center_crop
...
# ==========================================================
#  new: ensure the shift logic is robust
#  (even if the shift in util.util has problems, this can still be used)
# ==========================================================
def dt_ifftshift(x, dims=(-2, -1)):
    for dim in dims:
        x = torch.roll(x, shifts=-(x.size(dim)//2), dims=dim)
    return x

def dt_fftshift(x, dims=(-2, -1)):
    for dim in dims:
        x = torch.roll(x, shifts=(x.size(dim)//2), dims=dim)
    return x
# ==========================================================
...
# =========================================================================
#  fix the region: OPAT, OPA, OPATA
#  add dt_ifftshift / dt_fftshift to ensure the center data is compatible with corner FFT
# =========================================================================
class OPAT(nn.Module):
    # Sense Adjoint operator (K-space -> Image)
    ...
    def forward(self, k, mask):
        # Case A: Dataset (4D)
        if self.Smap.dim() == 4:
            num_coil, _, M, N = self.Smap.size()
            if mask.dim() == 3:
                mask = mask.unsqueeze(0).repeat(num_coil, 1, 1, 1)
            elif mask.dim() == 4 and mask.size(0) == 1:
                mask = mask.repeat(num_coil, 1, 1, 1)

            k_under = k * mask
            
            # fix: Center K -> Corner K -> IFFT -> Corner Img -> Center Img
            k_shifted = dt_ifftshift(k_under, dims=(-2, -1))
            im_u_shifted = ifft2(k_shifted.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
            im_u = dt_fftshift(im_u_shifted, dims=(-2, -1))
        ...
        # Case B: Training (5D)
            ...
            if mask.dim() < 5:
                mask = mask.view(BchSize, 1, 2, M, N).expand(BchSize, num_coil, 2, M, N)
                
            k_under = k * mask
            
            # fix: same as above
            k_shifted = dt_ifftshift(k_under, dims=(-2, -1))
            im_u_shifted = ifft2((k_shifted).permute(0, 1, 3, 4, 2)).permute(0, 1, 4, 2, 3)
            im_u = dt_fftshift(im_u_shifted, dims=(-2, -1))
...
class OPA(nn.Module):
    # Sense Forward operator (Image -> K-space)
    def __init__(self, Smap):
        super(OPA, self).__init__()
        self.Smap = Smap

    def forward(self, im, mask):
        # fix: Center Img -> Corner Img -> FFT -> Corner K -> Center K
        BchSize, num_coil, _, M, N = self.Smap.size()
        
        im = im.repeat(num_coil, 1, 1, 1, 1).permute(1, 0, 2, 3, 4)
        Image_s = complex_matmul(im, self.Smap)
        
        Image_s_shifted = dt_ifftshift(Image_s, dims=(-2, -1))
        k_full_shifted = fft2(Image_s_shifted.permute(0, 1, 3, 4, 2)).permute(0, 1, 4, 2, 3)
        k_full = dt_fftshift(k_full_shifted, dims=(-2, -1))
...
class OPATA(nn.Module):
    # Gram operator (Image -> K -> Image) for CG
    ...
    def forward(self, im, mask):
        ...
        Image_s = complex_matmul(im_coil, self.Smap) # Centered
        
        # Shift -> FFT -> Shift
        Image_s_shifted = dt_ifftshift(Image_s, dims=(-2, -1))
        k_full_shifted = fft2(Image_s_shifted.permute(0, 1, 3, 4, 2)).permute(0, 1, 4, 2, 3)
        k_full = dt_fftshift(k_full_shifted, dims=(-2, -1)) # Centered K
        
        # Masking
        mask_exp = mask.repeat(num_coil, 1, 1, 1, 1).permute(1, 0, 2, 3, 4)
        k_under = k_full * mask_exp
        
        # === Adjoint Part (K -> Img) ===
        # Shift -> IFFT -> Shift
        k_under_shifted = dt_ifftshift(k_under, dims=(-2, -1))
        Im_U_shifted = ifft2((k_under_shifted).permute(0, 1, 3, 4, 2)).permute(0, 1, 4, 2, 3)
        Im_U = dt_fftshift(Im_U_shifted, dims=(-2, -1)) # Centered Img
